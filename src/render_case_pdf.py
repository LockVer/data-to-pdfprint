...
def render_case_template_to_pdf(template, output_path):
    ...
    for page in template.pages:
        ...
        # 按照图片样式：5行表格，Quantity行包含数量和序列号
        row_heights = [50, 60, 80, 50, 50]  # 每行高度
        label_width = 150
        ...
        c.drawString(table_x + 10, current_y + (row_heights[0] - 14) / 2, "Item:")
        c.setFont("MicrosoftYaHei", 20)
        item_content = elements_dict.get("item", "Paper Cards")
        text_width = c.stringWidth(item_content, "MicrosoftYaHei", 20)
        content_x = table_x + label_width + (table_width - label_width - text_width) / 2
        c.drawString(content_x, current_y + (row_heights[0] - 20) / 2, item_content)
        ...
        c.setFont("MicrosoftYaHei", 14)
        c.drawString(table_x + 10, current_y + (row_heights[1] - 14) / 2, "Theme:")
        c.setFont("MicrosoftYaHei", 18)
        theme_content = elements_dict.get("theme", "")
        text_width = c.stringWidth(theme_content, "MicrosoftYaHei", 18)
        content_x = table_x + label_width + (table_width - label_width - text_width) / 2
        c.drawString(content_x, current_y + (row_heights[1] - 18) / 2, theme_content)
        
        # 第3行: Quantity (上半部分数量，下半部分序列号)
        c.line(table_x, current_y, table_x + table_width, current_y)
        current_y -= row_heights[2]
        c.line(table_x + label_width, current_y, table_x + label_width, current_y + row_heights[2])
        c.setFont("MicrosoftYaHei", 14)
        c.drawString(table_x + 10, current_y + (row_heights[2] - 14) / 2, "Quantity:")
        
        # 上半部分：数量
        quantity_content = elements_dict.get("quantity", "")
        c.setFont("MicrosoftYaHei", 16)
        text_width = c.stringWidth(quantity_content, "MicrosoftYaHei", 16)
        content_x = table_x + label_width + (table_width - label_width - text_width) / 2
        c.drawString(content_x, current_y + row_heights[2] - 25, quantity_content)
        
        # 下半部分：序列号范围
        serial_content = elements_dict.get("serial_range", "")
        c.setFont("MicrosoftYaHei", 14)
        text_width = c.stringWidth(serial_content, "MicrosoftYaHei", 14)
        content_x = table_x + label_width + (table_width - label_width - text_width) / 2
        c.drawString(content_x, current_y + 15, serial_content)
        ...
        c.setFont("MicrosoftYaHei", 14)
        c.drawString(table_x + 10, current_y + (row_heights[3] - 14) / 2, "Carton No.:")
        c.setFont("MicrosoftYaHei", 16)
        carton_content = elements_dict.get("carton_no", "")
        text_width = c.stringWidth(carton_content, "MicrosoftYaHei", 16)
        content_x = table_x + label_width + (table_width - label_width - text_width) / 2
        c.drawString(content_x, current_y + (row_heights[3] - 16) / 2, carton_content)
        ...
        c.setFont("MicrosoftYaHei", 14)
        c.drawString(table_x + 10, current_y + (row_heights[4] - 14) / 2, "Remark:")
        c.setFont("MicrosoftYaHei", 14)
        remark_content = elements_dict.get("remark", "")
        text_width = c.stringWidth(remark_content, "MicrosoftYaHei", 14)
        content_x = table_x + label_width + (table_width - label_width - text_width) / 2
        c.drawString(content_x, current_y + (row_heights[4] - 14) / 2, remark_content)
