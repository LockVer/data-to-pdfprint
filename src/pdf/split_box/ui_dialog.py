...
class SplitBoxUIDialog:
    ...
    def show_parameters_dialog(self):
        ...
        title_label = ttk.Label(
            center_container, text="分/套盒模板参数设置", font=("Arial", 14, "bold")
        )
        ...
        # 左侧：是否有小箱选择框架
        small_box_frame = ttk.LabelFrame(left_column, text="包装类型选择", padding="12")
        small_box_frame.pack(fill=tk.X, pady=(0, 10))
        ...
        has_small_box_radio.pack(side=tk.LEFT, padx=(0, 30))
        ...
        # 第二个参数：盒/套
        ttk.Label(params_frame, text="盒/套:", font=("Arial", 11)).grid(
            row=1, column=0, sticky=tk.E, pady=10, padx=(0, 15)
        )
        self.main_app.boxes_per_set_var = tk.StringVar()
        boxes_per_set_entry = ttk.Entry(
            params_frame, textvariable=self.main_app.boxes_per_set_var, width=20, font=("Arial", 11)
        )
        boxes_per_set_entry.grid(row=1, column=1, sticky=tk.W+tk.E, pady=10)

        # 第三个参数（根据选择动态调整）
        self.third_param_label = ttk.Label(params_frame, text="盒/小箱:", font=("Arial", 11))
        self.third_param_label.grid(row=2, column=0, sticky=tk.E, pady=10, padx=(0, 15))
        ...
        # 第四个参数（只在有小箱时显示）
        self.fourth_param_label = ttk.Label(params_frame, text="小箱/大箱:", font=("Arial", 11))
        self.fourth_param_label.grid(row=3, column=0, sticky=tk.E, pady=10, padx=(0, 15))
        self.main_app.small_boxes_per_large_box_var = tk.StringVar()
        self.fourth_param_entry = ttk.Entry(
            params_frame, textvariable=self.main_app.small_boxes_per_large_box_var, width=20, font=("Arial", 11)
        )
        self.fourth_param_entry.grid(row=3, column=1, sticky=tk.W+tk.E, pady=10)
    ...
    def confirm_parameters(self, dialog):
        ...
        # 获取参数值
        boxes_per_set_str = self.main_app.boxes_per_set_var.get().strip()
        ...
        try:
            # 尝试转换为数字
            boxes_per_set = int(boxes_per_set_str)
    ...
    def on_small_box_choice_changed(self):
        ...
        if has_small_box:
            # 有小箱：四级包装 (张/盒 -> 盒/套 -> 盒/小箱 -> 小箱/大箱)
            self.third_param_label.config(text="盒/小箱:")
            self.fourth_param_label.grid()
            self.fourth_param_entry.grid()
        else:
            # 无小箱：三级包装 (张/盒 -> 盒/套 -> 盒/大箱)
            self.third_param_label.config(text="盒/大箱:")
            self.fourth_param_label.grid_remove()
            self.fourth_param_entry.grid_remove()
