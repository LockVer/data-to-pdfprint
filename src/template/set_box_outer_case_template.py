...
专门用于生成套盒大箱标PDF，采用与盒标相同的格式：90x50mm标签
套盒大箱标特点：
- Item: 固定为 "Paper Cards"
- Quantity: 上方显示大箱总张数(套数×每套张数)，下方显示大箱包含的盒编号范围
- Carton No.: 显示套号范围 (如: 1-2，表示第1套到第2套)
"""
...
class SetBoxOuterCaseTemplate:
    ...
    def create_set_box_outer_case_label_data(self, excel_data, quantities):
        """
        根据Excel数据和套盒配置创建套盒大箱标数据
        ...
        # 计算大箱数：每个大箱装 sets_per_outer_case 套
        ...
        # 为每个大箱生成标签
        for outer_case_index in range(total_outer_cases):
            # 计算当前大箱包含的套范围
            start_set_index = outer_case_index * sets_per_outer_case
            end_set_index = min(start_set_index + sets_per_outer_case - 1, set_count - 1)
            
            # 计算当前大箱包含的套数
            current_sets_in_case = end_set_index - start_set_index + 1
            
            # 计算大箱总张数：套数 × 每套张数
            outer_case_total_sheets = current_sets_in_case * min_set_count
            
            # 计算当前大箱包含的盒范围
            start_box_index = start_set_index * boxes_per_set
            end_box_index = (end_set_index + 1) * boxes_per_set - 1
            # 确保不超过总盒数
            end_box_index = min(end_box_index, total_boxes - 1)
            ...
            if start_box_number == end_box_number:
                box_number_range = start_box_number
            else:
                box_number_range = f"{start_box_number}-{end_box_number}"
            
            # 套号范围
            if start_set_index == end_set_index:
                carton_no = f"{start_set_index + 1}"  # 单个套号
            else:
                carton_no = f"{start_set_index + 1}-{end_set_index + 1}"  # 套号范围
            ...
            clean_range = str(box_number_range).encode('utf-8').decode('utf-8') if box_number_range else 'JAW01001-01'
            clean_carton_no = str(carton_no).encode('utf-8').decode('utf-8') if carton_no else '1'
            clean_remark = str(excel_data.get('A4', '默认客户')).encode('utf-8').decode('utf-8')
            
            label_data = {
                'item': 'Paper Cards',  # 固定值
                'theme': clean_theme,  # 确保编码正确的主题
                'quantity': f"{outer_case_total_sheets}PCS",  # 大箱总张数
                'box_range': clean_range,  # 大箱包含的盒编号范围
                'carton_no': clean_carton_no,  # 套号范围
                'remark': clean_remark,  # 确保编码正确的备注(A4数据)
                'case_index': outer_case_index + 1,
                'total_cases': total_outer_cases
            }
            
            print(f"大箱 {outer_case_index + 1}: 套{start_set_index + 1}-{end_set_index + 1}, 盒{start_box_index + 1}-{end_box_index + 1}, 编号{box_number_range}, 套号范围{carton_no}, 总张数{outer_case_total_sheets}")
    ...
    def draw_set_box_outer_case_table_on_canvas(self, canvas_obj, label_data, x, y):
        """
        在Canvas上绘制套盒大箱标表格
        ...
        table_rows = [
            ...
            ('Quantity:', label_data.get('quantity', '7560PCS')),  # 大箱总张数
            ('', label_data.get('box_range', '')),  # 盒编号范围
            ('Carton No.:', label_data.get('carton_no', '1-2')),  # 套号范围
        ...
        for i, (label, content) in enumerate(table_rows):
            ...
            print(f"绘制套盒大箱标内容 {i}: '{clean_content}' 在位置 ({centered_x}, {row_y_center - 1 * mm})")
    
    def generate_set_box_outer_case_labels_pdf(self, excel_data, quantities, output_path):
        """
        生成套盒大箱标PDF
        ...
        """
        # 创建套盒大箱标数据
        outer_case_data = self.create_set_box_outer_case_label_data(excel_data, quantities)
        ...
        # 输出文件路径 - 套盒大箱标命名
        outer_case_file = label_folder / f"{clean_customer_name}+{clean_theme}+套盒大箱.pdf"
        ...
        # 设置PDF元数据
        c.setTitle(f"套盒大箱标 - {excel_data.get('B4', 'DEFAULT')}")
        c.setAuthor("数据转PDF打印工具")
        c.setSubject("90x50mm套盒大箱标批量打印")
        c.setCreator("套盒大箱标生成工具 v1.0")
        c.setKeywords("套盒大箱标,标签,PDF,打印")
        
        print(f"套盒大箱标页面布局: 90x50mm页面，每页1个标签")
        print(f"总计需要生成 {len(outer_case_data)} 个套盒大箱标")
        
        # 生成每个套盒大箱标
        for i, label_data in enumerate(outer_case_data):
            print(f"生成套盒大箱标 {i+1}/{len(outer_case_data)}: 套号范围{label_data.get('carton_no', f'{i+1}')}")
            
            # 在Canvas上直接绘制表格
            self.draw_set_box_outer_case_table_on_canvas(c, label_data, 0, 0)
        ...
        print(f"✅ 套盒大箱标PDF生成成功: {outer_case_file.name}")
        print(f"   总计生成 {len(outer_case_data)} 个套盒大箱标")
        
        return {
            'set_box_outer_case_labels': str(outer_case_file),
